...
- 프로젝트 구조 설계
```zsh
my-docker2/               # 프로젝트 루트 (전체 환경 관리)
├── Dockerfile            # 서버 이미지 빌드 레시피 (인프라 설정)
├── README.md             # 프로젝트 문서 (협업 및 가이드)
├── .gitignore            # Git 제외 목록 (불필요한 파일 방지)
└── practice-dir/         # 소스 코드 저장소 (바인드 마운트 대상)
    └── index.html        # 실제 서비스될 웹 콘텐츠
```
...
### [Case 2] 호스트 포트 충돌로 인한 컨테이너 실행 실패
...
- 도커 컨테이너는 호스트의 특정 포트를 빌려 써야 하는데, 이미 사용 중인 포트라 연결(Mapping)이 불가능한 상태임.

#### ③ 상태 확인
- 도커 컨테이너 확인: docker ps 명령어로 이미 8080 포트를 쓰고 있는 다른 컨테이너가 있는지 확인.
- 충돌 지점 발견: 이미 8080 포트를 점유하여 Up 상태인 다른 컨테이너가 있는지 체크.

#### ④ 해결 과정
- 방법 1 : 기존 컨테이너를 정리 한다.(동일한 포트를 사용하는 컨테이너가 있다면 docker stop 후 docker rm 수행)
